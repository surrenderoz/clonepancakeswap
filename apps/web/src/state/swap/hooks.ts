...
import { CAKE, USDC, USDT, STABLE_COIN, OKRT } from '@pancakeswap/tokens'
...
export function useDefaultsFromURLSearch():
  ...
  useEffect(() => {
    ...
    const parsed = queryParametersToSwapState(
      query,
      native.symbol,
      OKRT[chainId]?.address ?? STABLE_COIN[chainId]?.address ?? USDC[chainId]?.address ?? USDT[chainId]?.address,
    )
